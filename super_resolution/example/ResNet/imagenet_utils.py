...
def fbresnet_mapper(isTrain):
    ...
    def resize_shortest_edge(image, image_shape, size):
        ...
        shape = tf.cond(w_greater,
                        lambda: tf.cast(
                            [shape[0] / shape[1] * size, size], tf.int32),
    ...
    def center_crop(image, size):
        ...
        image = tf.slice(
            image, [offset_height, offset_width, 0], [size, size, -1])
        return image

    def lighting(image, std, eigval, eigvec):
        v = tf.random_uniform(shape=[3]) * std * eigval
        inc = tf.matmul(eigvec, tf.reshape(v, [3, 1]))
        image = tf.cast(tf.cast(image, tf.float32) +
                        tf.reshape(inc, [3]), image.dtype)
        return image
...
def eval_on_ILSVRC12(model, sessinit, dataflow):
    ...
    pred = FeedfreePredictor(pred_config, StagingInput(
        QueueInput(dataflow), device='/gpu:0'))
...
class ImageNetModel(ModelDesc):
    ...
    Either 'NCHW' or 'NHWC'
    """
    # channel first, N denotes the batch size
    data_format = 'NCHW'
    ...
    def build_graph(self, image, label):
        ...
        if self.loss_scale != 1.:
            logger.info(
                "Scaling the total loss by {} ...".format(self.loss_scale))
