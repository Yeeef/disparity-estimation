...
def resnet_basicblock(l, ch_out, stride):
    ...
    out = l + resnet_shortcut(shortcut, ch_out, stride,
...
def resnet_bottleneck(l, ch_out, stride, stride_first=False):
    ...
    l = Conv2D('conv1', l, ch_out, 1,
               strides=stride if stride_first else 1, activation=BNReLU)
    l = Conv2D('conv2', l, ch_out, 3,
               strides=1 if stride_first else stride, activation=BNReLU)
    l = Conv2D('conv3', l, ch_out * 4, 1, activation=get_bn(zero_init=True))
    out = l + resnet_shortcut(shortcut, ch_out * 4,
                              stride, activation=get_bn(zero_init=False))
    return tf.nn.relu(out)
...
def se_resnet_bottleneck(l, ch_out, stride):
    ...
    squeeze = FullyConnected('fc1', squeeze, ch_out //
                             4, activation=tf.nn.relu)
    squeeze = FullyConnected('fc2', squeeze, ch_out *
